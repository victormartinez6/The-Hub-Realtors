...
class SendGridFunctionService {
  /**
   * Envia um email usando a Cloud Function e SendGrid
   */
  async sendEmail(emailData: EmailData): Promise<EmailResult> {
    try {
      const functions = getFunctions();
      const sendEmailFunction = httpsCallable(functions, 'sendEmailViaSendGrid');
      
      // Log para depuração
      console.log('SendGridFunctionService: Enviando dados de email:', emailData);
  ...
  async sendTestEmail(toEmail: string): Promise<EmailResult> {
    try {
      const testEmailData: EmailData = {
        to: toEmail,
        subject: 'Teste de Envio de Email - The Hub Realtors',
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background-color: #f5f5f5; padding: 15px; border-radius: 5px; margin-top: 20px;">
              ...
              <ul style="list-style-type: none; padding-left: 0;">
                ...
                <li><strong>Método de envio:</strong> SendGrid via Cloud Functions</li>
              </ul>
            </div>
          </div>
        `
...
export const sendgridFunctionService = new SendGridFunctionService();
