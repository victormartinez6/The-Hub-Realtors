(function (exports, vue, shared, firestore, database) {
  'use strict';

  function useAuth(auth) {
    const user = vue.ref(auth.currentUser);
    const isAuthenticated = vue.computed(() => !!user.value);
    auth.onIdTokenChanged((authUser) => user.value = authUser);
    return {
      isAuthenticated,
      user
    };
  }

  function getData(docRef) {
    const data = docRef.data();
    if (data) {
      Object.defineProperty(data, "id", {
        value: docRef.id.toString(),
        writable: false
      });
    }
    return data;
  }
  function isDocumentReference(docRef) {
    var _a;
    return (((_a = docRef.path) == null ? undefined : _a.match(/\//g)) || []).length % 2 !== 0;
  }
  function useFirestore(maybeDocRef, initialValue = undefined, options = {}) {
    const {
      errorHandler = (err) => console.error(err),
      autoDispose = true
    } = options;
    const refOfDocRef = vue.isRef(maybeDocRef) ? maybeDocRef : vue.computed(() => maybeDocRef);
    let close = () => {
    };
    const data = vue.ref(initialValue);
    vue.watch(refOfDocRef, (docRef) => {
      close();
      if (!refOfDocRef.value) {
        data.value = initialValue;
      } else if (isDocumentReference(refOfDocRef.value)) {
        close = firestore.onSnapshot(docRef, (snapshot) => {
          data.value = getData(snapshot) || null;
        }, errorHandler);
      } else {
        close = firestore.onSnapshot(docRef, (snapshot) => {
          data.value = snapshot.docs.map(getData).filter(shared.isDef);
        }, errorHandler);
      }
    }, { immediate: true });
    if (autoDispose === true) {
      shared.tryOnScopeDispose(() => {
        close();
      });
    } else if (typeof autoDispose === "number") {
      shared.tryOnScopeDispose(() => {
        shared.useTimeoutFn(() => {
          close();
        }, autoDispose);
      });
    }
    return data;
  }

  function useRTDB(docRef, options = {}) {
    const {
      errorHandler = (err) => console.error(err),
      autoDispose = true
    } = options;
    const data = vue.ref(undefined);
    function update(snapshot) {
      data.value = snapshot.val();
    }
    const off = database.onValue(docRef, update, errorHandler);
    if (autoDispose)
      shared.tryOnScopeDispose(() => off());
    return data;
  }

  exports.useAuth = useAuth;
  exports.useFirestore = useFirestore;
  exports.useRTDB = useRTDB;

})(this.VueUse = this.VueUse || {}, Vue, VueUse, firebase, firebase);
